...
class YOLOL(L.LightningModule):
    ...
    def on_train_start(self):
        ...
        self.yolo_train.nw = (
            max(round(self.yolo_train.args.warmup_epochs * self.yolo_train.nb), 100)
            if self.yolo_train.args.warmup_epochs > 0
            else -1
        )  # warmup iterations
        ...
        LOGGER.info(
            ...
            + (
                ...
                else f"{self.yolo_train.epochs} epochs..."
            )
        )
        if self.yolo_train.args.close_mosaic:
            base_idx = (
                self.yolo_train.epochs - self.yolo_train.args.close_mosaic
            ) * self.yolo_train.nb
    ...
    def training_step(self, batch, i):
        ...
        if ni <= self.yolo_train.nw:
            ...
            self.yolo_train.accumulate = max(
                1,
                int(
                    np.interp(
                        ni, xi, [1, self.yolo_train.args.nbs / self.yolo_train.batch_size]
                    ).round()
                ),
            )
            for j, x in enumerate(self.yolo_train.optimizer.param_groups):
                ...
                if "momentum" in x:
                    x["momentum"] = np.interp(
                        ...
                        xi,
                        [
                            self.yolo_train.args.warmup_momentum,
                            self.yolo_train.args.momentum,
                        ],
                    )
        ...
        with torch.cuda.amp.autocast(self.yolo_train.amp):
            ...
            self.yolo_train.loss, self.yolo_train.loss_items = self.yolo_train.model(
                batch
            )
            if RANK != -1:
        ...
        if ni - last_opt_step >= self.yolo_train.accumulate:
            ...
            if self.yolo_train.args.time:
                self.yolo_train.stop = (
                    time.time() - self.yolo_train.train_time_start
                ) > (self.yolo_train.args.time * 3600)
        ...
        self.yolo_train.lr = {
            f"lr/pg{ir}": x["lr"]
            for ir, x in enumerate(self.yolo_train.optimizer.param_groups)
        }  # for loggers
    ...
    def on_train_epoch_end(self):
        ...
        if RANK in {-1, 0}:
            ...
            # Validation
            if (
                self.yolo_train.args.val
                or final_epoch
                or self.yolo_train.stopper.possible_stop
                or self.yolo_train.stop
            ):
                (
                    self.yolo_train.metrics,
                    self.yolo_train.fitness,
                ) = self.yolo_train.validate()
            self.yolo_train.metrics = {
            ...
            for key, value in self.yolo_train.metrics.items():
                self.log(key, value)
            self.yolo_train.stop |= (
                self.yolo_train.stopper(epoch + 1, self.yolo_train.fitness) or final_epoch
            )
            if self.yolo_train.args.time:
                self.yolo_train.stop |= (
                    time.time() - self.yolo_train.train_time_start
        ...
        if self.yolo_train.args.time:
            mean_epoch_time = (t - self.yolo_train.train_time_start) / (
                epoch - self.yolo_train.start_epoch + 1
            )
            self.yolo_train.epochs = self.yolo_train.args.epochs = math.ceil(
                self.yolo_train.args.time * 3600 / mean_epoch_time
            )
            self.yolo_train._setup_scheduler()
            self.yolo_train.scheduler.last_epoch = self.yolo_train.epoch  # do not move
            self.yolo_train.stop |= (
                epoch >= self.yolo_train.epochs
